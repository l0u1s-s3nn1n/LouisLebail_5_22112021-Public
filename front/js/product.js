/****-----Récupération et affichage des produits, ajout au panier(localstorage)-----*****/
  ...
  document.getElementById("addToCart").addEventListener("click", function () {
    ...
    if (
      ...
      if (parsedProduct) {
        ...
        const idAndColorExist = parsedProduct.find(
          (el) => el.id === idProduct && el.color === productColor
        );

        if (idAndColorExist) {
          //Additionne quantity dans optionsProduitsStockees à l'ancienne quantity (idAndColorExist)
          let addQuantity =
            parseInt(optionsProduitStockees.quantity) +
            parseInt(idAndColorExist.quantity);
          ...
          if (optionsProduitStockees.quantity == 1) {
            ...
            alert(
              `${productQuantity} ${productName} ont été ajoutés au panier!`
            );
