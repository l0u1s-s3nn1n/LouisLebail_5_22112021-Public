...
const calculPanier = () => {
  ...
  produitsDansLocalStorage.forEach(
    (el) => (totalQuantity += parseInt(el.quantity, 10))
  );
  ...
  let totalPrice = 0;
  produitsDansLocalStorage.forEach(
    (el) => (totalPrice += parseInt(el.price, 10) * parseInt(el.quantity, 10))
  );
...
const AfficherPanier = () => {
  ...
  if (produitsDansLocalStorage == null) {
    ...
    ).innerHTML = `<h1> Votre panier est vide</p>`;
    document.getElementById("totalQuantity").innerText = "0";
  ...
  // Si panier rempli
    ...
    for (let j in produitsDansLocalStorage) {
        ...
        return new Promise((resolve, reject) => {
          fetch(
            "http://localhost:3000/api/products/" +
              produitsDansLocalStorage[num].id
          )
            .then((response) => {
      ...
      promesse.push(
        getDataApiToDisplay(j).then((product) => {
          console.log(product);
    ...
    Promise.all(promesse).then(() => {
      for (let j in produitsDansLocalStorage) {
        ...
        createArticle.setAttribute(
          "data-color",
          produitsDansLocalStorage[j].color
        );
        ...
        createPriceP.innerText =
          produitsDansLocalStorage[j].price + ",00" + " €";
        ...
        createQuantityInput.setAttribute(
          "value",
          produitsDansLocalStorage[j].quantity
        );
        ...
        const deleteProduct = () => {
          ...
          for (let l = 0; l < deleteItem.length; l++) {
            deleteItem[l].addEventListener("click", () => {
              ...
              produitsDansLocalStorage = produitsDansLocalStorage.filter(
                (el) => el.id != deleteById || el.color != deleteByColor
              );
...
let regExName = new RegExp(
  "^[A-Za-z. 'àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ-]{2,20}$"
);
let regExAdress = new RegExp(
  "^[A-Za-z0-9. 'àèìòùÀÈÌÒÙáéíóúýÁÉÍÓÚÝâêîôûÂÊÎÔÛãñõÃÑÕäëïöüÿÄËÏÖÜŸçÇßØøÅåÆæœ-]{2,30}$"
);
...
const checkFirstName = (inputFirstName) => {
  ...
  if (checkfirstNameTest == true) {
    ...
    document.getElementById(
      "firstNameErrorMsg"
    ).innerText = `Veuillez renseigner un prénom valide SVP.`;
    return false;
  }
};
...
const checkLastName = (inputLastName) => {
  ...
  if (checklastNameTest == true) {
    ...
    document.getElementById(
      "lastNameErrorMsg"
    ).innerText = `Veuillez renseigner un nom de famille valide SVP.`;
    return false;
  }
};
...
const checkAddress = (inputAddress) => {
  ...
  if (checkAddressTest == true) {
    ...
    document.getElementById(
      "addressErrorMsg"
    ).innerText = `Veuillez saisir une adresse valide SVP.`;
    return false;
  }
};
...
const checkCity = (inputCity) => {
  ...
  if (checkCityTest == true) {
    ...
    document.getElementById(
      "cityErrorMsg"
    ).innerText = `Veuillez renseigner une ville valide SVP.`;
    return false;
  }
};
...
const checkEmail = (inputEmail) => {
  ...
  if (checkEmailTest == true) {
    ...
    document.getElementById(
      "emailErrorMsg"
    ).innerText = `Veuillez renseigner un e-mail valide SVP.`;
    return false;
  }
};
...
form.addEventListener("submit", function (e) {
  ...
  if (
    ...
    //----------Envoie informations au serveur----------
    //connection API methode POST
